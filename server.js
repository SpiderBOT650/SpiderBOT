...
function sendEmail(opts) {
  if (!process.env.RESEND_API_KEY) {
    console.warn('⚠️  RESEND_API_KEY not set — skip email');
    return Promise.resolve({ error: 'no api key' });
  }
...
// ── Email template ──
function emailTemplate(username, url) {
...
<div style="max-width:480px;margin:40px auto;padding:20px">
  <div style="background:#1A1733;border-radius:20px;padding:40px;border:1px solid rgba(139,92,246,0.3)">
    ...
    <p style="margin:0 0 28px;font-size:12px;color:#5A5680;letter-spacing:2px">AI TRADING BOT PLATFORM</p>
...
app.post('/api/register', async (req, res) => {
  ...
  if (!/^[a-zA-Z0-9_]+$/.test(username))
    return res.status(400).json({ error: '아이디는 영문·숫자·밑줄만 사용 가능합니다' });
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
  ...
  try {
    ...
    await sendEmail({
      ...
      html:    emailTemplate(username, `${base}/api/verify?token=${token}`),
    });

    res.json({ message: `${email}로 인증 메일을 발송했습니다. 확인 후 인증을 완료해주세요.` });
  } catch (e) {
    if (e.code === '23505') {
...
app.get('/api/verify', async (req, res) => {
  ...
  try {
    ...
    if (r.rowCount === 0) return res.redirect('/?error=expired');
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    if (!user.email_verified)
      return res.status(400).json({ error: '이메일 인증이 필요합니다. 받은 편지함을 확인해주세요', code: 'NOT_VERIFIED' });
...
app.post('/api/resend-verify', async (req, res) => {
  ...
  try {
    ...
    await sendEmail({
      ...
      html:    emailTemplate(user.username, `${base}/api/verify?token=${token}`),
    });
...
// ── Start ──
initDB()
  .then(() => app.listen(PORT, () => console.log(`🚀 SpiderBOT running on port ${PORT}`)))
